<template>
  ...
  >
    ...
    <FormItem label="模板">
      <Row>
        ...
        <Select v-model="formItem.templateName">
          <Option
            v-for="templateName in templateList"
            :value="templateName"
          >{{templateName}}</Option>
        </Select>
        </Col>
    ...
    <Row>
      ...
      <FormItem label="连接名">
        <Input
          v-model="formItem.entityConnName"
          placeholder=""
        ></Input>
    ...
    <Row>
      ...
      <FormItem label="中文文件名">
        <Row>
          ...
          <Button
            type="primary"
            @click="openDir(formItem.outputPath)"
          >打开目录</Button>
          </Col>
        </Row>
...
export default {
  ...
  methods: {
    async openDir(dir) {
      let vm = this;
      try {
        let response = await this.$http.get("/api/DataModel/openDir", {
          params: { dir: dir }
        });
    ...
    async genTable() {
      let vm = this;
      try {
        let response = await this.$http.get("/api/DataModel/GenTable", {
          params: vm.formItem
        });
        vm.$Message.info(response.data);
      } catch (error) {
    ...
    async getTemplateList() {
      let vm = this;
      try {
        let response = await vm.$http.get("/api/DataModel/GetTemplateList");
    ...
    async connect() {
      let vm = this;
      try {
        let response = await this.$http.get("/api/DataModel/Connect", {
          params: { connName: vm.formItem.connName }
        });
